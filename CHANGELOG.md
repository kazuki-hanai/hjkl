...
### Fixed
- Windows `hjkl enable` no longer depends on creating a scheduled task, which
  could fail with `Access is denied` for normal user sessions. It now installs
  a per-user Startup folder script and cleans up the v0.3.0 scheduled task on
  migration.
